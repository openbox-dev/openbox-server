...
export default function Box() {
    const model: THREE.Group = useLoader(GLTFLoader as any, '/assets/model/box.glb').scene;
    ...
    useEffect(() => {
        ...
        gsap.to(model.scale, { duration: 1, y: 1.5, x: 1.5, z: 1.5, onComplete: () => { canRotate = true; } })
    });
    ...
    useFrame((state, delta) => {
        ...
        if (startEyeAnimation > 4) {
            gsap.to(model.children[0].scale, { duration: 0.25, z: 0.05, ease: "back.out(1)", })
            gsap.to(model.children[0].scale, { duration: 0.25, z: 1, delay: 0.25, ease: "back.out(1)", })
            gsap.to(model.children[1].scale, { duration: 0.25, z: 0.05, ease: "back.out(1)", })
            gsap.to(model.children[1].scale, { duration: 0.25, z: 1, delay: 0.25, ease: "back.out(1)", })
            startEyeAnimation = 0;
        }
    });
    ...
    const handleClick = () => {
        if (canRotate) {
            canRotate = false;
            gsap.to(model.rotation, { duration: 1, y: model.rotation.y + Math.PI * 2, ease: "back.out(1)", onComplete: () => { canRotate = true; } })
        }
    }

    return <primitive position={[0, -7, 0]} rotation={[0, 0, 0]} object={model} onClick={handleClick} />
}
